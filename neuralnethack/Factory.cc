...
Error* Factory::createError(const Config& config, DataSet& data) {
	...
	if (config.errFcn() == SSE)
		error = new SummedSquare(*createMlp(config), data);
	else if (config.errFcn() == CEE)
		error = new CrossEntropy(*createMlp(config), data);
...
Trainer* Factory::createTrainer(const Config& config, DataSet& data) {
	...
	if (config.minMethod() == GD) {
		trainer =
		    new GradientDescent(error->mlp(), data, *error, MAX_ERROR, config.batchSize(),
		                        config.learningRate(), config.decLearningRate(), config.momentum());
	} else if (config.minMethod() == ADAM) {
		trainer = new Adam(error->mlp(), data, *error, MAX_ERROR, config.batchSize(),
		                   config.adamLearningRate(), config.adamBeta1(), config.adamBeta2(),
		                   config.adamEpsilon(), config.adamWeightDecay());
	} else {
...
DataTools::Sampler* Factory::createSampler(const Config& config, DataSet& data) {
	...
	} else if (config.ensParamDataSelection() == "none" ||
	           config.ensParamDataSelection() == "dummy") {
...
ModelEstimator* Factory::createModelEstimator(const Config& config, DataSet& data) {
	...
	} else if (config.msParamDataSelection() == "none" ||
	           config.msParamDataSelection() == "dummy") {
		sampler = new DataTools::DummySampler(data, config.msParamN());
	} else if (config.msParamDataSelection() == "hold") {
		sampler =
		    new DataTools::HoldOutSampler(data, config.msParamNumTrainingData(), config.msParamN());
	}
