...
static void writeUint32(ostream& os, uint32_t v) { os.write(reinterpret_cast<const char*>(&v), 4); }
static uint32_t readUint32(istream& is) { uint32_t v; is.read(reinterpret_cast<char*>(&v), 4); return v; }
static void writeDouble(ostream& os, double v) { os.write(reinterpret_cast<const char*>(&v), 8); }
static double readDouble(istream& is) { double v; is.read(reinterpret_cast<char*>(&v), 8); return v; }
...
void MultiLayerPerceptron::saveEnsembleBinary(const NeuralNetHack::Ensemble& ens, const string& path)
{
	ofstream os(path, ios::binary);
	if(!os) throw runtime_error("Cannot open file for writing: " + path);
