...
double SummedSquare::gradient(uint bs)
{
    double err=0;
...
double SummedSquare::outputError(uint bs)
{
    double err=0;
    for(uint i=0; i<bs; ++i){
	Pattern& p=theDset->nextPattern();
...
vector<double>& SummedSquare::gradient()
{
    theGradient = vector<double>(0);
    theGradient.reserve(theMlp->nWeights()); //Is this needed?

    for(uint i=0; i<theMlp->nLayers(); ++i)
	for(uint j=0; j<(*theMlp)[i].nNeurons(); ++j){
	    const vector<double>& g = (*theMlp)[i][j].gradients();
	    theGradient.insert(theGradient.end(), g.begin(), g.end());
	}

    assert(theGradient.size() == theMlp->nWeights());
