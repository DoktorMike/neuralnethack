...
void QuasiNewton::train(Mlp& mlp, DataSet& dset)
{
	...
	while(cntr-- && err > theTrainingError){
		...
		matrixTemp1 = G;
		mul(matrixTemp1,alpha);
		vectorTemp1=mul(matrixTemp1,g);
...
float QuasiNewton::err(Mlp& mlp, DataSet& dset, float alfa)
{
	...
	mul(vectorTemp1, alfa);
	add(vectorTemp1,w,vectorTemp1); //vectorTemp1=w+alfa*G*g;

	mlp.weights(vectorTemp1); //set weights.
