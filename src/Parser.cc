	...
	while(!in.eof()){
		...
		if(in.peek() == '%')
			...
			if(in.good()){
				...
				if(token == "FileNameT")
					config.fileNameT = parseFileName(in); 
				else if(token == "InColT")
					config.inColsT = parseCol(in); 
				else if(token == "OutColT")
					config.outColsT = parseCol(in); 
				else if(token == "PType")
...
string Parser::parsePType(ifstream& in)
{
	string buf;
	in>>buf;
	return buf;
...
void Parser::parseMSParam(ifstream& in, Config& config)
{
	...
	double buf2;
	in>>buf2;
	config.msParamNumTrainingData = buf2;
}
