	...
	while(!in.eof()){
		...
		if(in.peek() == '%')
			...
			if(in.good()){
				...
				if(token == "Suffix") in>>config.suffix;
				else if(token == "FileName") config.fileName = parseFileName(in); 
				else if(token == "InCol") config.inCols = parseCol(in); 
				else if(token == "OutCol") config.outCols = parseCol(in); 
				else if(token == "FileNameT") config.fileNameT = parseFileName(in); 
				else if(token == "InColT") config.inColsT = parseCol(in); 
				else if(token == "OutColT") config.outColsT = parseCol(in); 
				else if(token == "PType") config.problemType = parsePType(in); 
				else if(token == "NLay") config.nLayers = parseNLay(in); 
				else if(token == "Size") config.arch = parseSize(in, config.nLayers); 
				else if(token == "ActFcn") config.actFcn = parseActFcn(in, config.nLayers); 
				else if(token == "ErrFcn") config.errFcn = parseErrFcn(in); 
				else if(token == "MinMethod") config.minMethod = parseMinMethod(in); 
				else if(token == "MaxEpochs") config.maxEpochs = parseMaxEpochs(in); 
				...
				else if(token == "SaveSession") in>>config.saveSession;
				else if(token == "Info") in>>config.info;
				else if(token == "SaveOutputList") in>>config.saveOutputList;
				else if(token == "Seed") in>>config.seed;
				token = "";
			}
		}
	}
...
bool Parser::parsePType(ifstream& in)
{
	string buf;
	in>>buf;
...
{
	in>>config.msParamDataSelection;
	in>>config.msParamN;
	in>>config.msParamK;
	string mode = "";
	in>>mode;
	config.msParamSplitMode = (mode == "rnd") ? true : false;
	in>>config.msParamNumTrainingData;
}

void Parser::parseEnsParam(ifstream& in, Config& config)
{
	in>>config.ensParamDataSelection;
	in>>config.ensParamN;
	in>>config.ensParamK;
	string mode = "";
	in>>mode;
	config.ensParamSplitMode = (mode == "rnd") ? true : false;
	in>>config.ensParamNewWeights;
}

void Parser::parseMSGParam(ifstream& in, Config& config)
{
	uint buf;
	in>>buf;
	config.msgParamN = (uint)buf;
	in>>buf;
